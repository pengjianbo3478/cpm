	...
	<!--右侧主内容start-->
	 ...
	 <table width="100%" border="0" >
	  ...
	  <c:forEach var="item" items="${paginate.data}" varStatus="s"> 
	 	<tr>
		   <td>
		      <c:if test="${item.shortName == '' && assets.shortName ==null}">${item.assetsName}</c:if>
		   	  <c:if test="${item.shortName != '' || assets.shortName !=null}">${item.shortName}</c:if>
		   </td>
...
<input type="hidden" id="_DETAIL" value="${ctx}/assetsdetail/search.do">
